...
```
Usage: outlet-daemon <start|stop|restart> [options]

When calling outlet without <start|stop|restart> the daemon will start as a foreground process
...
```
AWS_REGION='eu-west-1' outlet start [other options]
```
